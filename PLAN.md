...
- [x] **The always-visible POSITION strip is deliberately not HAAS-sized.** On the
      ...
      **39.2px, 3.2× the original**, with the pane's own width and height both
      ...
      each axis letter hugging its own number so the slack falls between groups.
      **Sized for eight characters, not for what is on screen.** This machine's
      travel is 200mm, so `-200.000` is reachable: it is 175px of the 180 left in
      the cell after the letter.
      **Width is the limit, not height.** Four eight-character readings plus their
      letters across a 781px pane caps the type at ~36px; the pane would allow
      ~59px on height alone. Doubling it would need the pane twice as wide or
